...
function parseDateInput(s, allDay) {
    if (allDay) {
...
async function cmdSetup(argv) {
    ...
    if (!appleId || !password)
        die("setup requires --apple-id and --password (or --password-file)");
...
async function cmdList(argv) {
    ...
    if (values.json) {
        ...
        for (const e of events) {
            ...
            if (e.location)
                process.stdout.write(`    location: ${e.location}\n`);
            if (e.description)
                process.stdout.write(`    notes: ${e.description.replace(/\n+/g, " ")}\n`);
        }
    }
}
...
async function cmdUpdate(argv) {
    ...
    if (values.title !== undefined)
        patch.summary = values.title;
    if (values.desc !== undefined)
        patch.description = values.desc;
    ...
    if (Object.keys(patch).length === 0)
        die("update requires at least one of --title, --start, --end, --desc, --location, --allday");
    const client = new CalendarClient({ calendarName: values.calendar });
...
main();
//# sourceMappingURL=claw-cal.js.map