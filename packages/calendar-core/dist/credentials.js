...
export function getCredentials() {
    ...
    if (envUser && envPass)
        return { appleId: envUser, password: envPass };
    ...
    if (result.status !== 0 || !acctMatch || !passMatch) {
        throw new Error(`iCloud CalDAV credentials not found. Run \`claw-cal setup\`, or set ICLOUD_APPLE_ID and ICLOUD_APP_PASSWORD env vars.`);
    }
    return {
        appleId: acctMatch[1],
...
export function stashCredentials(appleId, password) {
    const result = spawnSync("security", ["add-generic-password", "-U", "-s", KEYCHAIN_SERVICE, "-a", appleId, "-w", password], { encoding: "utf-8" });
    if (result.status !== 0) {
