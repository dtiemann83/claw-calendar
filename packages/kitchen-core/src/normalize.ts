...
function toSingular(s: string): string {
  ...
  if (s.endsWith("s") && s.length > 1 && !s.endsWith("ss")) return s.slice(0, -1)
  return s
}
 ...
 */
export function resolveItem(db: Database.Database, raw: string): ResolvedItem {
  ...
  const byAlias = db
    .prepare(
      `SELECT items.id AS id, items.canonical_name AS canonicalName
         FROM item_aliases
         JOIN items ON items.id = item_aliases.item_id
        WHERE item_aliases.alias = ?`,
    )
    .get(normalized) as { id: number; canonicalName: string } | undefined
  ...
  if (singular !== normalized) {
    const bySingular = db
      ...
      .get(singular) as { id: number; canonicalName: string } | undefined
    if (bySingular) {
      // Learn the new alias for next time.
      db.prepare(`INSERT OR IGNORE INTO item_aliases (alias, item_id) VALUES (?, ?)`).run(
        normalized,
        bySingular.id,
      )
  ...
  const insert = db
    .prepare(
      `INSERT INTO items (canonical_name, is_staple, created_at) VALUES (?, 0, datetime('now')) RETURNING id`,
    )
