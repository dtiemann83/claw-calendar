...
function cmdAdd(argv) {
    ...
    if (!text)
        die("add requires the item text as a positional argument");
...
function cmdBought(argv) {
    ...
    const text = positionals.join(" ").trim();
    if (!text)
        die("bought requires the item text as a positional argument");
    const kc = KitchenClient.open();
...
function cmdOut(argv) {
    ...
    const text = positionals.join(" ").trim();
    if (!text)
        die("out requires the item text as a positional argument");
    const kc = KitchenClient.open();
    kc.markOut({ text, by: values.by });
    process.stdout.write(`OK out "${text}"\n`);
}
function cmdLow(argv) {
    const { values, positionals } = parseArgs({
        args: argv,
        options: { by: { type: "string" } },
        allowPositionals: true,
    ...
    const text = positionals.join(" ").trim();
    if (!text)
        die("low requires the item text as a positional argument");
    const kc = KitchenClient.open();
...
main();
//# sourceMappingURL=claw-kitchen.js.map